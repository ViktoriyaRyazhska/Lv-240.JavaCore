...
public class UScurrency {
	...
	public static void main(String[] args) {
		...
		String str = "Logic: Con$ider first character in thi$ string with $ $ymbol$ and start counting occurrence of $ and other symbols besides $";

		int count = 0;
		String temp = ""; // An empty string to keep track of counted characters
		...
		for (int i = 0; i < str.length(); i++) {
			...
			for (int j = i; j < str.length(); j++) {
				...
				char k = str.charAt(j);
				// take one character (c) and compare with each character (k) in
				// the string
